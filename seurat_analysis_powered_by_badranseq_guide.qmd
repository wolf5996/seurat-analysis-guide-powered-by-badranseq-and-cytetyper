...
- Every visualization step provides a side-by-side comparison between Seurat defaults and BadranSeq alternatives via tabsets
...
`do_StatsViolinPlot()` adds an omnibus Kruskal-Wallis test annotation by default. Since we have a single group here, `pairwise.display = "none"` suppresses pairwise comparisons.
...
# QC violin plots with statistical annotations ----------
...
cell_data <- fetch_cell_data(pbmc)
...
`do_PcaPlot()` automatically annotates axes with the percentage of variance explained by each PC — information that Seurat's default `DimPlot` omits. Cell borders and cluster labels are enabled by default.
...
`EnhancedElbowPlot()` adds a red vertical cutoff line at the selected PC and displays variance explained on the y-axis, making the dimensionality choice explicit.
...
EnhancedElbowPlot(pbmc, ndims = 30, cutoff_pc = 10)
```
:::
...
`do_UmapPlot()` adds cell borders, cluster labels with white fill backgrounds, and uses `generate_badranseq_colors()` for a vivid categorical palette by default.
...
`do_FeaturePlot()` uses viridis color scaling (palette B, magma) by default with cell borders. `order = TRUE` ensures high-expression cells are plotted on top.
...
`do_StatsViolinPlot()` overlays an omnibus Kruskal-Wallis test. `pairwise.display = "none"` shows only the global test statistic without cluttering with all pairwise comparisons.
